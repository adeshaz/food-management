...
// app/dashboard/orders/success/page.tsx - FIXED
'use client';
...
function OrderSuccessContent() {
    ...
    const fetchOrderWithFallback = async (id: string) => {
        try {
            ...
            try {
                ...
                if (recentOrders) {
                    const orders = JSON.parse(recentOrders);
                    const found = orders.find((o: any) => o.orderNumber === id || o._id === id);
            ...
            return {
                ...
                items: [{ name: 'Food Item', quantity: 1, price: 1000, total: 1000 }]
            };
        }
    };
    ...
    const fetchOrderDetails = async (id: string) => {
        try {
            ...
            const items: OrderItem[] = (orderData.items || []).map((item: any) => ({
                name: item.foodItem?.name || item.name || 'Food Item',
                quantity: item.quantity || 1,
                price: item.price || 0,
                total: (item.quantity || 1) * (item.price || 0)
            }));
            ...
            <div className="container mx-auto px-4">
                <div className="max-w-4xl mx-auto">
                    <div className="text-center mb-8">
                        ...
                        <h1 className="text-4xl font-bold text-gray-900 mb-3">Order Successful! 🎉</h1>
                        <p className="text-xl text-gray-600">Thank you for your order. We'll start preparing it right away.</p>
                    </div>
                        ...
                        <CardContent className="p-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                <div className="bg-green-50 p-4 rounded-lg">
                                    ...
                                    <p className="text-3xl font-bold text-green-700">₦{order.totalAmount?.toLocaleString() || '0'}</p>
                                    <div className="mt-2 flex items-center justify-between text-sm">
                                        <span className="text-gray-600">Payment:</span>
                                ...
                                <div className="bg-blue-50 p-4 rounded-lg">
                                    ...
                                    <div className="mt-2 space-y-1 text-sm text-gray-600">
                                        <div className="flex items-center gap-2"><User className="h-4 w-4" />{order.contactName}</div>
                                        <div className="flex items-center gap-2"><Phone className="h-4 w-4" />{order.contactPhone}</div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="border rounded-lg">
                                    ...
                                    {order.items?.map((item, idx) => (
                                        <div key={idx} className="grid grid-cols-12 gap-4 p-4 border-t">
                                            <div className="col-span-6">{item.name}</div>
                                            <div className="col-span-2 text-center">{item.quantity}</div>
                                            <div className="col-span-2 text-right">₦{item.price.toLocaleString()}</div>
                                            <div className="col-span-2 text-right font-medium">₦{item.total.toLocaleString()}</div>
                                    ...
                                    <div className="grid grid-cols-12 gap-4 p-4 border-t bg-green-50 font-bold">
                                        <div className="col-span-8">Total Amount</div>
                                        <div className="col-span-4 text-right text-green-700">₦{order.totalAmount?.toLocaleString() || '0'}</div>
                                    </div>
                                </div>

                                <div className="flex justify-between items-center p-4 bg-gray-50 rounded-lg">
                                    <div>
                                        <div className="flex items-center gap-2"><Clock className="h-5 w-5 text-blue-600" /><span className="font-medium">Estimated Delivery</span></div>
                                        <p className="text-sm text-gray-600 mt-1">{order.restaurant.name} will deliver by {formattedTime}</p>
                                    </div>
                                    <Badge className={order.status === 'pending' ? 'bg-yellow-100 text-yellow-800' : order.status === 'preparing' ? 'bg-blue-100 text-blue-800' : order.status === 'delivered' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}>
                                        {order.status?.toUpperCase()}
                                    </Badge>
                                </div>
                            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                        <Button asChild className="h-auto py-3"><Link href="/dashboard/orders"><Package className="h-5 w-5 mr-2" /> View All Orders</Link></Button>
                        <Button asChild variant="outline" className="h-auto py-3"><Link href="/foods"><ShoppingBag className="h-5 w-5 mr-2" /> Order More Food</Link></Button>
                        <Button asChild variant="ghost" className="h-auto py-3"><Link href="/"><Home className="h-5 w-5 mr-2" /> Back to Home</Link></Button>
                    </div>
                    ...
                    <Card className="mb-8">
                        ...
                        <CardContent>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <h4 className="font-medium mb-2 flex items-center gap-2"><MessageSquare className="h-5 w-5 text-blue-600" /> Contact Information</h4>
                                    <ul className="space-y-2 text-sm text-gray-600">
                                        <li>📞 Call: <strong>+234 801 234 5678</strong></li>
                                        <li>✉️ Email: <strong>support@foodmanagement.com</strong></li>
                                        <li>🏢 Restaurant: <strong>{order.restaurant.name}</strong></li>
                                ...
                                <div>
                                    <h4 className="font-medium mb-2 flex items-center gap-2"><Share2 className="h-5 w-5 text-purple-600" /> Quick Actions</h4>
                                    <div className="space-y-2">
                                        <Button variant="outline" className="w-full justify-start" onClick={() => { navigator.clipboard.writeText(`Order #${order.orderNumber} - ₦${order.totalAmount?.toLocaleString()}`); toast.success('Order details copied!'); }}>Copy Order Details</Button>
                                        <Button variant="outline" className="w-full justify-start" onClick={() => window.print()}>Print Receipt</Button>
                                        <Button variant="outline" className="w-full justify-start" onClick={handleTrackOrder}>Track This Order</Button>
                                    </div>
                                </div>
                            </div>
                        </CardContent>
                    ...
                    {order.paymentStatus === 'pending' && (
                        <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg mb-6">
                            <div className="flex items-center gap-2 mb-2"><Clock className="h-5 w-5 text-yellow-600" /><h4 className="font-semibold text-yellow-800">Payment Pending</h4></div>
                            <p className="text-sm text-yellow-700">Your order is confirmed but payment is pending. Please complete your payment to start food preparation.</p>
                        </div>
                    )}
                </div>
            </div>
...
export default function OrderSuccessPage() {
    return (
        <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><Loader2 className="h-12 w-12 animate-spin text-green-600" /></div>}>
            <OrderSuccessContent />
        </Suspense>
    );
}